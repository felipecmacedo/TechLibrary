...
public class FilterBookUseCase
{
    ...
    public ResponseBooksJson Execute(RequestFilterBooksJson request)
    {
        ...
        var books = query
            .OrderBy(book => book.Title) //Ordena primeiramente pelo Título dos livros
            .ThenBy(book => book.Author) //Depois ordena por Autores dos livros
            .Skip(skip) //Pula a quantidade de elementos
            .Take(PAGE_SIZE) //Pega os próximos até um determinado limite  
            .ToList();
